...
// RN port of frontend/src/components/BeachDetail/CommentSection/hooks/useComments.ts, verbatim —
// no DOM/RN-specific dependency in here, just fetch + state.
export interface UseCommentsResult {
  comments: CommentWithAuthor[];
  /** True only until the *first* fetch settles — refetches (after a post/delete) update
   * `comments` in place without flipping this back on, so the count heading never flashes back
