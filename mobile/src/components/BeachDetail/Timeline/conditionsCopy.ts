...
// RN port of frontend/src/components/BeachDetail/Timeline/conditionsCopy.ts, verbatim. Beaufort
// force names ("moderate breeze") are precise but mean nothing to a swimmer deciding whether to go
// in — this is the plain-language gloss shown for each one. Shared between Verdict's synthesized
// sentence and SeaConditions' itemized rows, so the two never drift into describing the same wind
// differently.
