...
export interface ReportFlagState {
  ...
  // Whether Timeline should render its inline flag picker at all — true for both an eligible
...
/**
 * RN port of frontend/src/components/BeachDetail/ReportFlag/hooks/useReportFlag.ts — same state
 * machine, verbatim (auth/eligibility/submission), just AuthContext/AUTO_DISMISS_MS imported from
 * their RN paths. Owns the whole "report the flag" feature end to end so Timeline can stay
 * presentational and just render whatever this returns. Picking a color submits immediately for a
 * signed-in eligible visitor, or remembers the pick and opens AuthScreen for a signed-out one,
 * auto-submitting once they're authenticated.
 *
 * `enabled` is the isUnguarded === false gate Timeline applies (mirroring frontend's own gate on
 * the old ReportFlagButton): pass false while that's still unknown (undefined) or true, and every
 * flag here stays inert rather than flashing the report flow on for a beach it doesn't apply to.
 */
