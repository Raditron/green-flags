...
/**
 * Fetches the signed-in visitor's saved Beaches as full records for the Saved page's grid.
 * Deliberately separate from SavedBeachesContext, which only tracks saved ids (see its class
 * doc) — the page reads this hook for the Beach data to render each card, and reads the context
 * live for which of those ids are *currently* saved, so an unsave from this page's own star
 * drops the card without needing to refetch this list.
 */
